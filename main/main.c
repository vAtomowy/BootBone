...
while (1) {
    ...
    // 1) Brak PROV_DONE jeszcze – tryb AP + webserver działa
    if (!(bits & PROV_DONE) && !(xEventGroupGetBits(bootbone_s) & ON_STA_MODE)) {
        ESP_LOGI(TAG, "Bootbone waiting on fill form");
    }

    // 2) Ustaw przejście do trybu STA po zapisaniu formularza
    if ((bits & PROV_DONE) && !(xEventGroupGetBits(bootbone_s) & ON_STA_MODE)) {
        xEventGroupSetBits(bootbone_s, ON_STA_MODE);
        ESP_LOGI(TAG, "Set Bootbone mode to STA");
    }

    // 3) Jeżeli mamy ON_STA_MODE, a jeszcze nie zaczęliśmy łączenia, to wykonaj przełączenie i zacznij czekać
    ...
    // 5) Praca w stanie online
    if (xEventGroupGetBits(bootbone_s) & CONNECTED_TO_AP) {
        static bool logged = false;
        if (!logged) {
            ESP_LOGI(TAG, "Device is ONLINE (STA, IP acquired)");
            logged = true;
        }
    }
